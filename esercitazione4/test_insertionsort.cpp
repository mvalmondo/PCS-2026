...
int main() {
	...
	if (is_sorted(s)) {
			std::cout << "InsertionSort test sulla stringa: PASSED" << "\n";
	...
	for (int n=4; n<=8192; n*=2 ) {
		...
		tc.tic();
		insertion_sort(v1);
		double tempo_alg_insertion=tc.toc();
		
		tc.tic();
		std::sort(v2.begin(), v2.end());
		double tempo_sort=tc.toc();
		std::cout<< "Tempo impiegato dall'algoritmo insertion: "<< tempo_alg_insertion << "\n";
...
//il std::sort della libreria standard è piu veloce dell'algoritmo dell'ordinamento per i vettori grandi